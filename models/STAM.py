...
class STAM(nn.Module):
    ...
    def forward(self, feat_map):
        ...
        #print(feat_map.size(),'feat_map.size()')#torch.Size([8, 8, 1024, 7, 7]) feat_map.size()

        reshape_map = feat_map.view(b * t, c, h, w)
        feat_vect = self.avg(reshape_map).view(b, t, -1)
        #print(feat_vect.shape,'feat_vect')#torch.Size([8,4, 1024]) feat_vect
        ...
        # print('gap_feat_map0',gap_feat_map0.shape) torch.Size([8, 4, 1024, 7, 7])
        gap_feat_map = self.SRAG(feat_map, reshape_map, embed_feat, feat_vect, gap_feat_map0)
        # print('gap_feat_map',gap_feat_map.shape) torch.Size([32, 1024, 7, 7])
        gap_feat_map = self.conv_block(gap_feat_map)
        gap_feat_map = gap_feat_map.view(b, -1, c, h, w)
        # print('gap_feat_map',gap_feat_map.shape) torch.Size([8, 4, 1024, 7, 7])
